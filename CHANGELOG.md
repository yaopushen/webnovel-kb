...
| 版本 | 日期 | 说明 |
| :--- | :--- | :--- |
| **v1.13.0** | **2026-06-10** | **重构初稿生成（模仿+创造） + 搜索洞察双层语义去重**（详见下方） |
...
**初稿生成重构（模仿模式 + 创造模式）：**
- ✅ **重构 generate_sample 为 generate_draft** — 原有“文抄公范例生成”工具全面升级重构为“初稿生成”工具。
- ✅ **模仿模式 (`imitate`)** — 仅保留了原先改写幅度最大且效果最好的 `moderate`（中度改写）逻辑，并大幅调优了重写提示词（严禁机械同义词对拷）与生成参数（`temperature=0.7`），实现真正的深度改写。
- ✅ **创造模式 (`create`)** — 接入 `mimo-v2.5-pro` 思考链大模型。首创在 `assistant` 角色中采用“硬编码种子 + 动态传入章节原文”的双风格种子注入，辅以文风锚点与网文控制规则，根据 Agent 传入的大纲创作出 3000-4000 字的高品质初稿，并完整回显思考链与正文。
- ✅ **API 配置与智能搜索对齐** — 创造模式接口默认直接与智能搜索模块绑定的 `LLM_CHAT_API_KEY` 与 `LLM_CHAT_BASE_URL` 动态同步，避免多套餐冗余配置。同时，网络请求采用 `trust_env=False` 强力规避了网络环境中的无效 SOCKS 代理报错。
- ✅ **过度自信警示** — 工具 docstring 及手册中增设“过度自信警示”，提示并指引 Agent 写作工具实际输出质量通常比 Agent 自行预期的更高，应给予充分信任。

**搜索洞察（Agent 知识层）双层自动去重：**
- ✅ **双层去重机制** — 优化了 `KnowledgeStore.add` 入库接口，增加了双层过滤防线：
  - **精确标题查重**：快速比对入库标题，若为相同 query 触发的智能搜索洞察则跳过写入直接返回，防止数据膨胀。
  - **语义向量查重**：利用 `DEDUP_THRESHOLD = 0.95` 进行同类别知识相似度匹配，相似度在 95% 以上直接过滤查重，安全回显历史条目。
- ✅ **Embedding 向量重用** — 在查重环节中计算的文本向量 `emb` 会在真正写入数据库时被重用，免除了二次 API 向量计算，使知识入库写入效率提高一倍。
