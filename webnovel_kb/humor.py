...
class HumorExtractor:
    ...
    async def extract(self, texts: List[str], metas: List[dict], exact_title: str) -> List[Dict[str, Any]]:
        """提取幽默场景。"""
        ...
        for idx in sample_indices:
            ...
            try:
                response = await self.chat.chat(messages, temperature=0.1, max_tokens=1024)
                if response:
                    matches = re.findall(
                        r'\("humor"<\|>(.+?)<\|>(.+?)<\|>(.+?)\)', response
                    )
