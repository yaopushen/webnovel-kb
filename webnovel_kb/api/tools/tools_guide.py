...
## 10. generate_sample
- **功能**：范例生成（文抄公模式）。针对指定的经典章节进行不同精细度的词句改写，生成可直接参考的模仿文本。
- **参数**：
  - `novel_title` (str, 必填): 书名。
  - `chapter` (int, 必填): 章节号。
  - `rewrite_level` (str, 默认 `"minimal"`):
    - `"minimal"`: 极微调，仅替换人名地名，保留95%的句式。
    - `"light"`: 微调细节与环境，保留90%句式。
    - `"moderate"`: 调整情节小走向，保留80%的核心骨架。
  - `custom_instructions` (str, 可选): 自定义改写指令。
  - `preserve_elements` (list, 可选): 强制要求不可改写的元素列表（如 `["语言风格"]`）。
