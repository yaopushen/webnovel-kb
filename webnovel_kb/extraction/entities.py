...
class EntityExtractor:
    ...
    def extract(self, novel_title: str, novel_id: str, exact_title: str,
        ...
        """快速提取实体（单chunk模式）。"""
        if not self.chat:
            raise ExtractionError("Chat API未配置，无法自动提取实体。请设置LLM_API_KEY。")
    ...
    def extract_cross_chunk(self, novel_title: str, novel_id: str, exact_title: str,
        ...
        """跨章节提取实体（带时间线分析）。"""
        if not self.chat:
            raise ExtractionError("Chat API未配置")
        ...
        for source, target, desc, strength in rel_matches:
            ...
            if source and target:
                try:
                    ...
                    logger.debug(f"Failed to add relationship {source}->{target}: {e}")
        ...
        for match in rel_matches:
            ...
            if source and target and rel_type:
                try:
                    ...
                    logger.debug(f"Failed to add cross-chunk relationship {source}->{target}: {e}")
